...
const createGantt=(n)=>{
   ...
   // creat gantt to display 
   document.getElementById("gantt").style.display = "block";
   let tan_name, tan_value;

   for(let i = 0; i < n; i++){
      for(let j = i+1; j < n ; j++){
         if(hrrn[i].ct > hrrn[j].ct){

            tan_value = hrrn[i].ct;
            tan_name = hrrn[i].name;
            hrrn[i].ct = hrrn[j].ct;
            hrrn[i].name = hrrn[j].name;
            hrrn[j].ct = tan_value;
            hrrn[j].name = tan_name;
         }
      }
   ...
   for(let i = 0; i < n; i++){
      ...
      const td_name = document.createElement("tr");
      const td_ct = document.createElement("tr");

      document.getElementById("gtbody").appendChild(gt);

      td_name.textContent = hrrn[i].name;
      td_ct.textContent = hrrn[i].ct;

      gt.appendChild(td_name);
      gt.appendChild(td_ct);
   }
}
...
const getDataHRRN_RR=(quantum)=>{
   ...
   if(arrival.length != burst.length) {
      ...
      while(time < sum_bt + hrrn[0].at){
         ...
         // set time quantum -> ทำงานตาม quantum
         count = 0;
